            ...
            <ul class="nav navbar-top-links navbar-right">
                ...
                <li class="dropdown">
                   <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                        ...
                        $sql=mysql_fetch_array(mysql_query("select count(*) as notif from Barang where status_notif=1 or status_notif =6"));
                        echo $sql['notif']
                        ?> </b><i class="fa fa-caret-down"></i>
                    </a>
                    <ul class="dropdown-menu dropdown-alerts">
                            <?php
                            $resultNotif=mysql_query("select * from Barang join pesanan using (Id_Pesanan) where status_notif=1 ");
                            while ($row=mysql_fetch_array($resultNotif,MYSQL_BOTH)) {
                                echo '<li class="divider"></li>
                                <li>
                                    <a href="'.site_url().'/Kain/vKain/'.$row[0].'/status">
                                        <div>
                            ...
                            <?php
                            $resultNotif=mysql_query("select * from Barang join pesanan using (Id_Pesanan) where status_notif=6 ");
                            while ($row=mysql_fetch_array($resultNotif,MYSQL_BOTH)) {
                                echo '<li class="divider"></li>
                                <li>
                                    <a href="'.site_url().'/Kain/vdatakain/'.$row[0].'/status">
                                        <div>
                ...
                <!-- /.dropdown -->
                        ...
                        $sql=mysql_fetch_array(mysql_query("select username as akun from akun where roll='kain'"));
                        echo $sql['akun']
                        ?></i></li>
                        <li><i class=""></i><?= anchor("manajer/logout","Log Out") ?></li>
                <!-- /.dropdown -->
            </ul>
            <!-- /.navbar-top-links -->

           <?php echo $this->load->view('navigator_menu/tampilan_bagkain')?>
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">

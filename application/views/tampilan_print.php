...
<body>
    ...
    <div id="wrapper">
        ...
        <nav class="navbar navbar-default navbar-static-top" role="navigation" style="margin-bottom: 0">
            ...
            <ul class="nav navbar-top-links navbar-right">
                ...
                <li class="dropdown">
                   <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                        ...
                        $sql=mysql_fetch_array(mysql_query("select count(*) as notif from barang  where status_notif=2 or status_notif = 7" ));
                        echo $sql['notif'] 
                        ?> </b><i class="fa fa-caret-down"></i>
                    </a>
                    <ul class="dropdown-menu dropdown-alerts">
                            <?php
                            $resultNotif=mysql_query("select * from barang join Pesanan using (Id_Pesanan)  where status_notif=2 ");
                            while ($row=mysql_fetch_array($resultNotif,MYSQL_BOTH)) {
                                echo '<li class="divider"></li>
                                <li>
                                    <a href="'.site_url().'/Perint/vPrint/'.$row[0].'/status">
                                        <div>
                                            <strong style="color:orange">1 Barang Pesanan Baru  '.$row[1].'</strong>
                            ...
                            ?>
                             ...
                             <?php
                            $resultNotif=mysql_query("select * from pesanan join barang using (Id_Pesanan) where status_notif = 7");
                            while ($row=mysql_fetch_array($resultNotif,MYSQL_BOTH)) {
                                echo '<li class="divider"></li>
                                <li>
                                    <a href="'.site_url().'/Perint/vdataprint/'.$row[0].'/status">
                                        <div>
                                            <strong style="color:orange">Pesanan Produksi '.$row[0].'</strong>
                                            <span class="pull-right text-muted">
                                            </span>
                                        </div>
                                    <div>'.$row[8].' dengan nama '.$row[9].'</div>
                                    </a>
                                </li>';
                            }
                            ?>
                ...
                <!-- /.dropdown -->
                        ...
                        $sql=mysql_fetch_array(mysql_query("select username as akun from akun where roll='perint'"));
                        echo $sql['akun']
                        ?></i></li>                       
                         <li><i class=""></i><?= anchor("perint/logout","Log Out") ?>
           ...
           <?php echo $this->load->view('navigator_menu/tampilan_bagprint')?>
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">

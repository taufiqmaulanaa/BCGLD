...
<body>
    ...
    <div id="wrapper">
        ...
        <nav class="navbar navbar-default navbar-static-top" role="navigation" style="margin-bottom: 0">
            ...
            <ul class="nav navbar-top-links navbar-right">
                ...
                <li class="dropdown">
                   <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                        ...
                        $sql=mysql_fetch_array(mysql_query("select count(*) as notif from barang where status_notif=3 or status_notif=8"));
                        echo $sql['notif']
                        ?> </b><i class="fa fa-caret-down"></i>
                    </a>
                    <ul class="dropdown-menu dropdown-alerts">
                            <?php
                            $resultNotif=mysql_query("select * from barang join pesanan using (Id_Pesanan) where status_notif= 3");
                            while ($row=mysql_fetch_array($resultNotif,MYSQL_BOTH)) {
                                echo '<li class="divider"></li>
                                <li>
                                    <a href="'.site_url().'/Jahit/vJahit/'.$row[0].'/status">
                                        <div>
                            ...
                            <?php
                            $resultNotif=mysql_query("select * from barang join pesanan using (Id_Pesanan) where status_notif= 8");
                            while ($row=mysql_fetch_array($resultNotif,MYSQL_BOTH)) {
                                echo '<li class="divider"></li>
                                <li>
                                    <a href="'.site_url().'/Jahit/vdataJahit/'.$row[0].'">
                                        <div>
                                            <strong style="color:orange">Pesanan Produksi '.$row[0].'</strong>
                                            <span class="pull-right text-muted">
                                            </span>
                                        </div>
                                    <div>'.$row[1].' dengan nama '.$row[2].'</div>
                                    </a>
                                </li>';
                            }
                            ?>
                ...
                <!-- /.dropdown -->
                        ...
                        $sql=mysql_fetch_array(mysql_query("select username as akun from akun where roll='jahit'"));
                        echo $sql['akun']
                        ?></i></li>  
                        <li>
                            <i class=""></i><?= anchor("perint/logout","Log Out") ?>
           ...
           <?php echo $this->load->view('navigator_menu/tampilan_bagjahit')?>
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
